...
def test_compose_has_one_gpu_owner_and_private_vllm_namespace() -> None:
    compose = yaml.safe_load((ROOT / "docker-compose.yml").read_text(encoding="utf-8"))
    services = compose["services"]
    devices = services["vllm-server"]["deploy"]["resources"]["reservations"]["devices"]
    assert devices == [{"driver": "nvidia", "count": 1, "capabilities": ["gpu"]}]
    assert services["proxy"]["network_mode"] == "service:vllm-server"
    assert "deploy" not in services["proxy"]
    assert "8000:8000" not in (ROOT / "docker-compose.yml").read_text(encoding="utf-8")
...
def test_the_bench_service_runs_upstreams_benchmark_through_the_proxy() -> None:
    ...
    # about an agentic request's prefill shape or tool-schema overhead. vLLM's
    # built-in BFCL loader carries each sample's tool schema over the chat backend
    # and applies the template itself, so there is no `--skip-chat-template`.
    assert command[command.index("--backend") + 1] == "openai-chat"
    assert command[command.index("--endpoint") + 1] == "/v1/chat/completions"
    assert command[command.index("--dataset-name") + 1] == "hf"
    assert (
        command[command.index("--dataset-path") + 1]
        == "gorilla-llm/Berkeley-Function-Calling-Leaderboard"
    )
    assert (
        command[command.index("--bfcl-categories") + 1]
        == "simple,multiple,parallel,parallel_multiple"
    )
    assert "--skip-chat-template" not in command
